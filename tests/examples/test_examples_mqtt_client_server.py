...
def test_mqtt_clean_session():
    ...
    try:
        ...
        client1 = Manager()
        conn1 = client1.mqtt_connect(f"mqtt://127.0.0.1:{port}", clean_session=True, keepalive=5)
        ...
        client2 = Manager()
        conn2 = client2.mqtt_connect(f"mqtt://127.0.0.1:{port}", clean_session=False, keepalive=5)
