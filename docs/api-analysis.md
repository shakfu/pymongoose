...
### Short Term (2-3 PRs)
5. **MQTT basic support** - connect/listen/pub/sub (huge use case for Python)
6. **Multipart forms** (`mg_http_next_multipart`) - File uploads
7. **URL encoding** (`mg_url_encode`) - Complete URL utilities
8. **Timer API** (`mg_timer_add`) - Periodic tasks without threads
9. **HTTP status/header parsing** (`mg_http_status`, `mg_http_get_header_var`)

### Medium Term (Future releases)
10. **DNS resolution** (`mg_resolve`) - Async DNS
11. **SNTP** - Time synchronization
12. **RPC framework** - Built-in RPC server
13. **TLS configuration** - Production TLS support
14. **Base64/hashing utilities** - Common crypto needs

### Long Term (As needed)
15. Advanced connection management
16. File system abstraction
17. Low-level crypto (ECC, RSA)
